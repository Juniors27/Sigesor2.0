...
export default function BandejaExtemporaneosPage() {
  ...
  const fetchFechasDisponibles = async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/bandeja/extemporaneos/fechas-disponibles/`)
  ...
  const fetchFuas = async (filters = {}, page = pagination.page, pageSize = pagination.pageSize) => {
    ...
    try {
      ...
      const url = `http://127.0.0.1:8000/api/bandeja/extemporaneos/${params.toString() ? `?${params.toString()}` : ""}`
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, "FuasExtemporaneos.xlsx")
  }
  ...
  /*const handleEditRow = (id, currentEstado) => {
    setEditingRow(id)
    setTempEstado(currentEstado);
  }*/
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-semibold mb-4">Bandeja Extemporáneos</h1>
